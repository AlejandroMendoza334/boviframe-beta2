...
class _ConsultaAnimalScreenState extends State<ConsultaAnimalScreen> {
  ...
  void initState() {
    super.initState();
    // Al entrar a la pantalla, inmediatamente cargamos todas las evaluaciones:
  ...
  /// ordenadas por timestamp descendente. Además, extrae “numero_sesion”
  /// desde el documento padre en “sesiones/<sessionId>”.
  Future<void> _cargarTodasLasEvaluaciones() async {
    ...
    try {
      // 1) Usamos collectionGroup para traer todos los doc de "evaluaciones_animales"
      final querySnapshot = await FirebaseFirestore.instance
          .collectionGroup('evaluaciones_animales')
          .orderBy('timestamp', descending: true)
          .get();

      // 2) Para cada documento de evaluación, vamos a buscar su padre “sesiones/<sessionId>”
      final List<Future<Map<String, dynamic>>> futuros = querySnapshot.docs.map((doc) async {
        final data = doc.data() as Map<String, dynamic>;

        // Insertamos el evalId (por si se necesita luego)
        data['evalId'] = doc.id;

        // Por defecto, si algo falla, número de sesión vacío:
        String numeroSesionPadre = '—';

        // Intentamos obtener el padre en “sesiones/<sessionId>”
        final parentRef = doc.reference.parent.parent;
        if (parentRef != null) {
          final sesionSnap = await parentRef.get();
          if (sesionSnap.exists) {
            final sesionData = sesionSnap.data() as Map<String, dynamic>?;
            // Aquí esperamos que, al crear la sesión, guardaras “numero_sesion”: "Sesion X"
            numeroSesionPadre = (sesionData?['numero_sesion'] ?? '—').toString();
          }
        }
      ...
      // 3) Esperamos a que todas las peticiones asíncronas terminen:
      final listaConSesiones = await Future.wait(futuros);

      setState(() {
        _resultados = listaConSesiones;
        _loading     = false;
      });
  ...
  /// Si ambos campos están vacíos, recarga todo (misma lógica de init).
  Future<void> _buscarAnimales() async {
    ...
    try {
      ...
      // 4) Mismo proceso: obtenemos “numero_sesion” de cada documento padre:
      final List<Future<Map<String, dynamic>>> futurosFiltrados =
      ...
      final listaFiltradaConSesiones = await Future.wait(futurosFiltrados);

      setState(() {
        _resultados = listaFiltradaConSesiones;
        _loading     = false;
      });
    }
                            ...
                            child: ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  // En lugar de session_id, mostramos numero_sesion:
                                  Text(
